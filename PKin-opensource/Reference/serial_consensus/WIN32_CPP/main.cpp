...
extern void readCosts(const char * filename);
...
extern void doAutoConsensus(const char * filename, int locicount);
extern void doAutoGreedyConsensus(const char * filename, int locicount);
...
void testConsensus()
{
  ...
  char ** p = (char**)calloc(7, sizeof(char*));
  p[0]= f1;
  p[1] = f2;
  p[2] = f3;
  p[3] = f4;
...
void test3( )
{
  ...
  for(i = 0; i < max; i ++)
      ...
      for(j =0; j < nloci; j ++)
	{
	  ...
	  //	  for(k=1; k < MAX_ALLELES_PER_LOCUS; k ++)
	    {
	      ...
	      *value2 = k++;
	      ...
	      int * key3 = (int*)malloc(sizeof(int));
	      ...
	      *value3 = k++;
	      ...
	      int * key4 = (int*)malloc(sizeof(int));
      ...
      sibgroup * fish ;
      if((fish = (sibgroup*)hashtable_search(sibgroupsmap, group)) == NULL || fish->mutate != i)
	{
...
void test( )
{
  while (1)
    {
      sibgroup * test = (sibgroup*)malloc(sizeof(sibgroup));
...
// vanilla non-consensus should be: <exe> test <nloci> <input> <output_setfile>
