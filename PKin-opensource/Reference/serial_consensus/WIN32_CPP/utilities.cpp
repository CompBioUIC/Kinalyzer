...
void populateMoveMap()
{
  ...
  // APR - changing int to char*
  char* read = fgets(temp, 512, infile);
  char * x, *y, *z;
  char * delims = ",;\n ";
  // APR - removing strtok_r - why do we need the reentrant version?
  //char * progress;
...
void generateInitialGroups()
{
  ...
  for(i = 0; i < individuals; i ++, current = (*current).next )
      ...
      for(j = i+1; j < individuals; j++, current2 = (*current2).next)
	{
	  ...
	  calculateHash(group);
	  sibgroup * exist;
	  if(NULL == (exist = (sibgroup*)hashtable_search(sibgroupsmap, (void*)group)))
	    {
...
void sortLociByAlleleFrequency()
{
  int i = 0;
  int j = 0;
  lociorder = (int*)calloc(nloci, sizeof(int));
 for( i = 0; i < nloci; i ++)
   {
     lociorder[i] = i;
   }
...
int sibgroupEquals(void * group1, void * group2)
{
  ...
  if((*((sibgroup*)group1)).hash != (*((sibgroup*)group2)).hash)
    return 0;

  for ( i = 0; i < nloci; i ++)
    {
      if(hashtable_count(((sibgroup*)group1)->intmaps[i]) != hashtable_count(((sibgroup*)group2)->intmaps[i]))
	return 0;

      itr = hashtable_iterator((*((sibgroup*)group1)).intmaps[i]);
	...
	int *v2;
	if(NULL == (v2 = (int*)hashtable_search((*((sibgroup*)group2)).intmaps[i], key) ))
	  {
	    free(itr);
	    return 0;
	  }
	...
	{
	  if((*((sibgroup*)group1)).possibilitysets[i][j] != (*((sibgroup*)group2)).possibilitysets[i][j])
	    {
	      return 0;
	    }
	}
...
void init()
{
  ...
  costs = (double*)calloc(7, sizeof(double));
 ...
 *************************************************/
int pairEquals(void * p1, void * p2)
{
  if((*((Pair*)p1)).x == (*((Pair*)p2)).x && (*((Pair*)p1)).y == (*((Pair*)p2)).y)
    {
      return 1;
    }
  return 0;
 ...
 ***********************************************************/
// APR - casting int to unsigned int to match hash signature
...
void readPossibilities(const char * filename)
{
  ...
  // APR - removing strtok_r - why do we need this to be reentrant?
  /*
  char * progress;
  */
  while(infile)
...
void readData(const char * filename)
{
  ...
  while(infile)
	...
	{
	  // APR - REFACTOR - this can be made into a single function to handle each allele (lots of duplication)
	  ...
	  *k1 = pair->x;
	  ...
	  if(val1 == NULL)
	    ...
	    free(k1);
	  (*val1)++;
	  // APR - Is this a bug? - k1 should be k2 here shouldn't it? - Changing this to be k2
	  int * val2 = (int*)hashtable_search(alleles[index], /*k1*/k2);
          if(val2 == NULL)
            {
...
void readSelectedData(const char * filename, int droplocus)
{
  ...
  while(infile)
      ...
      if(read ==NULL || strlen(buffer) < 2)
	{
	  ...
	  // APR - Refactor - tons of duplicate code for each allele
...
{
  if(source->assignments->head!=NULL && source->assignments->head->previous!= NULL)
    {
      int breaker = 1;
    }
  ...
  if(group->assignments->head!=NULL && group->assignments->head->previous!= NULL)
    {
      int breaker = 1;
    }
...
void deleteSibgroup(void * group)
{
  deleteSibgroupShallow((sibgroup*)group, 0);
}
  ...
  for( i = 0; i < nloci; i ++)
    {
      ...
      /*struct hashtable_itr * itr = hashtable_iterator(group->intmaps[i]);
            do{
...
// APR - Refactor: BEGIN FUNCTION GetMappedAllele(...)
...
// APR - END FUNCTION - GetMapped Allele ... now called GetMappedAllele() again below for 'y' allele
