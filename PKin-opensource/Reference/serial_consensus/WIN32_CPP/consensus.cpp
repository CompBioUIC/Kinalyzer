...
void blank(void*);
// APR - changing return type from int to unsigned int to match hash function pointer signature
unsigned int getAssignmentBasedGroupHash(void * group);
int equalsAssignmentGroups(void * group1, void * group2);
...
// APR - additional externs
extern int isEmptyIntersection(int * intersection);
extern int equalsPossibilitySets(int * set1, int * set2);
extern void addToSortedList(list * addTo, int x);
extern void sortLociByAlleleFrequency();
extern int searchList(list * l, int x);
extern void readPossibilities(const char * filename);
extern void reconstruct();
extern void writeGamsFile(const char * filename, struct hashtable * sets);
...
double getMergeCost(sibgroup * group1, sibgroup * group2)
{
  ...
  for(index = group2->assignments->head; index != NULL && cost <= costs[MAX_GROUP_EDIT] && cost <= ceil(costs[MAX_ERROR_RATE] *totalinds);
      index = index->next)
    {
	  // APR - casting void* to int
      for( ;((individual*)indi->data)->index != (int)index->data && indi != NULL; indi =indi->next)
	{
	  // nthn
	}
        if(smoothmerge)
	{
	  ...
	  if(assignIndividualToGroup(merge, (individual*)indi->data) >0 )
	    {
	      bcost += getAssignmentBenefit(merge, (individual*)indi->data);
	  ...
	  else
	    {
	      ...
	      smoothmerge = 0;
	      icost  = getAssignmentCost(merge, (individual*)indi->data);
	      if(icost <= maxedit)
		cost += icost  ;
	      else 
		cost += INFINITY;
...
double getAssignmentCost(sibgroup * group, individual * indi)
{
	...
	for(loc = 0; loc < nloci && cost <= maxedit; loc ++)
	{
	  ...
	  int *xlkup, *ylkup;
	  xlkup= (int*)hashtable_search(allelemappings, &(indi->loci[locus].x));
	  ylkup = (int*)hashtable_search(allelemappings, &(indi->loci[locus].y));
	  if(NULL == xlkup
	     && NULL == ylkup)
	    {
...
{	
  ...
  list * sets= (list*)malloc(sizeof(list));
  sets->head = NULL;
  sets->count = 0;
  char * read;
  //  char buff[512];
  char * pbuff = (char*)calloc(512, sizeof(char)); //buff;
  int id = 0;
  read = fgets(pbuff, 512, setfile);
  while( setfile && read != NULL)
    {
      sibgroup * itemset= (sibgroup*)malloc(sizeof(sibgroup));
...
void populateAssignmentSets(list * sets)
{
  ...
  for( ; ptr != NULL ; ptr = ptr->next)
	...
	{
	// APR - converting void* to int...
	  for(; ((individual*)data.current->data)->index < (int)iptr->data && data.current!= NULL; icount ++)
	    {
	      data.current = data.current->next;
	    }
	  int result = assignIndividualToGroup(group, (individual*)data.current->data);
	  if(result == 0)
	    {
	      printf("In Set: ");
...
list vote(list * setlists, int votecount)
{
  ...
  final->head = NULL;
  final->count = 0;
  int ** votes = (int**)calloc(data.count, sizeof(int*));
  int i, j, k;
  node * p1, *p2;
  int * v4 = (int*)calloc(data.count, sizeof(int));
  sibgroup * p3;
  list * disputed = (list*)malloc(sizeof(list));
  disputed->head = NULL;
  disputed->count = 0;
  for(i = 0;i < data.count; i++)
    {
      votes[i] = (int*)calloc(data.count, sizeof(int));
    }
  for(p1 = setlists->head;p1 != NULL; p1 = p1->next)
    { // each setlist
      printf("new soln\n");
      int ** v2 = (int**)calloc(data.count, sizeof(int*));
      for(i = 0; i < data.count; i ++)
	{
	  v2[i] = (int*)calloc(data.count, sizeof(int));
	}
      int * v3 = (int*)calloc(data.count, sizeof(int));
  ...
  int * itemset = (int*)calloc(data.count, sizeof(int));
  for(j = 0; j < data.count; j ++ )
    {
      itemset[j] = j;
    }
  int * freebies = (int*)calloc(data.count, sizeof(int));
  ...
  list * temp = (list*)calloc(data.count, sizeof(list));
  for(j = 0; j <data.count ; j ++)
    {
      temp[j].head= NULL;
      temp[j].count = 0;
  ...
  for(j = 0 ; j < data.count; j ++)
	...
	sibgroup * group = (sibgroup*)malloc(sizeof(sibgroup));
	initializeSibgroup(group);
	group->assignments->head = (node*)malloc(sizeof(node));
	// APR - casting int to void*...
	group->assignments->head->data= (void*)j;
...
void doConsensus(char * datafile,  char ** setfiles, int count)
{
  ...
  list * sets = (list*) result.head->data;
  populateAssignmentSets(sets);
  list * disputed = (list*) result.head->next->data;
...
void printResults(const char * filename, list * sets)
{
  ...
  for( ; p != NULL; p = p->next)
      ...
      for(counter = 0; counter < data.count; counter ++)
	{
		...
		// APR - casting void* to int...
	  if(indnode != NULL && counter == (int)indnode->data)
	    {
...
void assignDisputed(list * sets, list * disputed)
{
  ...
  for(p = disputed->head; p != NULL; p = p->next, icount ++)
    {
      individual * jones = (individual*)p->data;
      node * q;
      ...
      for(q = sets->head, scount = 0; q !=NULL ; q= q->next, scount ++ )
	{
	  ...
	  double cost = 0.0;
	  sibgroup * g = copySibgroup((sibgroup*)q->data);
	  if(assignIndividualToGroup(g, (individual*)p->data))
	    {
	      cost = getAssignmentBenefit((sibgroup*)q->data, (individual*)p->data);
	    }
	  else
	    {
	      cost = getAssignmentCost((sibgroup*)q->data, (individual*)p->data);
	    }
	  if(dolog)
	    {
      ...
      if(mincost <= maxedit )
	{
	  ...
	  copy->edit += mincost;
	  addToSortedList(copy->assignments,((individual*) p->data)->index);*/
	  assignWithEdit(assignedGroup, (individual*)p->data);
	...
	{
	  sibgroup *singleton = (sibgroup*)malloc(sizeof(sibgroup));
	  initializeSibgroup(singleton);
	  assignIndividualToGroup(singleton, (individual*)p->data);
...
void mergeGreedyGroups(list * groups)
{
  ...
  do{
    ...
    if(optcost <= costs[MAX_GROUP_EDIT] && optcost <= ceil(costs[MAX_ERROR_RATE] * totalinds))
      {
	sibgroup * merge = (sibgroup*)copySibgroup((sibgroup*)opt1->data);
	node * iptr = data.head;
	...
	for(ptr = ((sibgroup*) opt2->data)->assignments->head ; ptr != NULL ; ptr = ptr->next)
	  {
	    ...
	    if(optcost < merge->edit)
	      {
	  // APR - casting void* to int..
		for(; iptr != NULL && i != (int)ptr->data; iptr= iptr->next, i ++);
		assignIndividualToGroup(merge, (individual*)iptr->data);
	      }
		// APR - casting void* to int..
	    addToSortedList(merge->assignments, (int)ptr->data);
	  }
	...
	deleteSibgroup((sibgroup*)opt1->data);
	...
	//opt2 = temp;
	deleteSibgroup((sibgroup*)opt2->data);
	free(opt2);
	groups->count --;
	merging = 1;
      }
...
void mergeGroups(list * groups)
{
  ...
  for( ; group1!= NULL; group1 = group1->next)
	...
	{
	  sibgroup * merge = copySibgroup((sibgroup*)group1->data);
	  sibgroup * g = (sibgroup*)group2->data;
	  int giveup = 0;
	  int i = 0;
	  node * ind;
	  node * assignedind;
	  for ( ind = data.head, assignedind = g->assignments->head; assignedind != NULL && i < individuals && ! giveup; ind = ind->next, i ++)
	    {
		// APR - converting void* to int
	      if((int)assignedind->data == i)
		{
		  int result = assignIndividualToGroup(merge, (individual*)ind->data);
	  ...
	  if(!giveup)
		...
		group1->previous->next = mnode;
	      ...
	      deleteSibgroup((sibgroup*)group1->data);
...
[m,n] = size(adat);*/
  ...
  //  struct hashtable_itr * itr1, * itr2;
  iter = hashtable_iterator(allconsensusgroups); 
  
  for( i=1; i <= m; i ++)
   {
     sibgroup * group = (sibgroup*)hashtable_iterator_key(iter);
     fprintf(fid, "t%d",i);
     for(j = 1 ; j <= n; j ++)
...
void doAutoConsensus(const char * filename, int locicount)
{
  ...
  nloci = locicount - 1;
  char ** filenames = (char**)calloc(locicount, sizeof(char*));
  //
  //  readData(filename);
  list source = data;
  int droppedlocus = 0;
  for( droppedlocus = 0; droppedlocus < locicount; droppedlocus ++)
    {
      ...
      char * solnname = (char*)calloc(512, sizeof(char));
...
void doAutoGreedyConsensus(const char * filename, int locicount)
{
  ...
  for( droppedlocus = 0; droppedlocus < locicount; droppedlocus ++)
      ...
      if(pmovemap[1][1] == 0)
	{
      ...
      strcpy(lstname, solname);
      strcat(lstname, ".lst");
	  // APR - directing to perl
      strcpy(solncmd, "perl extract-solution-param.pl ");
      strcat(solncmd, lstname);
      system(solncmd);
	  ...
	  // APR - calling write script
      char writecmd[512];
      strcpy(writecmd, "perl write-solution.pl -i ");
      strcat(writecmd, solname);
	  strcat(writecmd, " -s ");
	  strcat(writecmd, solname);
	  strcat(writecmd, ".sol ");
	  strcat(writecmd, "-o ");
	  strcat(writecmd, solnname);
	  system(writecmd);

      //hashtable_destroy(possibilitiesMap, 0);
      for(i = 0; i < nloci; i ++)
        {
          hashtable_destroy(alleles[i], 0);// = (struct hashtable *) create_hashtable(10, getIntHash, equals);
        }
      free( alleles);
...
void generateAllConsensusGroups(list * groups)
{
  ...
  allconsensusgroups = (struct hashtable *) create_hashtable(groups->count * groups->count, 
    ...
    for(group1 = groups->head; group1 != NULL; group1 = group1->next)
      {
	sibgroup * gg = (sibgroup*)group1->data;
  ...
  do{
	...
	for(group2 = group1->next; group2 != NULL; group2 = group2->next )
	  {
		...
		if(g!= NULL && g->edit > merge->edit)
		  ...
		  sibgroup * g;
		  if( (NULL == (g = (sibgroup*)hashtable_search(allconsensusgroups, merge))) ||
		      g->edit > merge->edit)
		    {
    ...
    do {
      ...
      sibgroup * gp1 = (sibgroup*)hashtable_iterator_key(itr1);
//	for(group2 = group1->next; group2 != NULL; group2 = group2->next )
      do  {
	if(hashtable_count(newbiestable) == 0)
	  continue;
	sibgroup * gp2 = (sibgroup*)hashtable_iterator_key(itr2);
	optcost = 9999;
	sibgroup * gp, *ngp;
	    double mcost = getMergeCost(gp1, gp2);
	    double mcost2 = getMergeCost(gp2, gp1);
    ...
    do{
      sibgroup * gp1 = (sibgroup*)hashtable_iterator_key(itr1);
      sibgroup * gp2 = (sibgroup*)hashtable_search(allconsensusgroups, gp1);
...
void dropLocus(int locusdrop, list sourcedata)
{
  ...
  for( ; current != NULL; current = current->next)
    {
      dataentry * e = (dataentry*)current->data;
      Pair * loci = (Pair*)calloc(nloci, sizeof(Pair));
      dataentry *ne = (dataentry*)malloc(sizeof(dataentry));
      //      *ne = *e;
...
// APR - changing return type from int to unsigned int to match type signatures for hashing function pointer
unsigned int getAssignmentBasedGroupHash(void * group)
{
  if(hashtable_count(((sibgroup*)group)->intmaps[0]) == 0)
    printf("ERROR: empty allele maps");
  int hash = 0;
  node * ptr = ((sibgroup*)group)->assignments->head;

  for( ; ptr != NULL; ptr = ptr->next)
    {
      hash ^= ( (((int)ptr->data )<< (((int)ptr->data) % 8)) ^ (((int)ptr->data) >> (((int)ptr->data) %8)));
    }
  // APR - cast to unsigned int
...
int equalsAssignmentGroups(void * group1, void * group2)
{
  ...
  if(hashtable_count(((sibgroup*)group1)->intmaps[0]) == 0)
    printf("ERROR: empty allele maps");
 

  if(hashtable_count(((sibgroup*)group2)->intmaps[0]) == 0)
    printf("ERROR: empty allele maps");

  node * p1, * p2;
  p1 = ((sibgroup*)group1)->assignments->head;
  p2 = ((sibgroup*)group2)->assignments->head;
