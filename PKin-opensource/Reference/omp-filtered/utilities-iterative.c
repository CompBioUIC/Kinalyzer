...
/********************************************************
 ********************************************************/
void intersectsibgroups(list * sibgroupslist, list * currentList)
{
  int nthr; 
  time_t walltime;
  time_t start;
  ...
  //  int i;
  int thr_id=0;
  int i,counter2,startindex,sgcount,ecount,index;
  sgcount = sibgroupslist->count;
  struct hashtable ** localtables;
  localtables = calloc(20,sizeof(struct hashtable *));
...
#pragma omp parallel for private(thr_id,copy,zlist,index,ecount,xptr,yptr,zptr,counter,xlist,ylist,empty,sgcurrent,startindex,counter2) firstprivate(heap,heapsize) \
  shared(blocks,sibgroupslist, currentList, intersection,nthr_checked,ptrs,ptrs2,localtables)//  threadprivate(ptrs,ptrs2)// schedule(dynamic)  //#pragma omp parallel for private(copy,zlist,xptr,yptr,zptr,counter,xlist,ylist,empty,sgcurrent) shared(blocks,ptrs,sibgroupslist, currentList, intersection,nthr_checked) schedule(dynamic) 
  for(counter = 0; counter < currentList->count; counter ++) 
      ...
      if(localtables[thr_id] == NULL)
	{
      ...
      for(counter2 = startindex+1; counter2 != startindex ; counter2 = (counter2 +1)%sgcount)
	{
	  ...
	  //#pragma omp critical
	  {
	    if(!empty && NULL == hashtable_search(localtables[thr_id], zlist))
	      {
		
		ecount = hashtable_count(localtables[thr_id]);
		//		index = (ecount*nthr*blocks)+(thr_id*blocks);
		index = (heapsize/nthr)*thr_id+(ecount*blocks);
		if(index > heapsize)
		  {

		    printf("Increase heapsize\n");
		    exit(-1);
		  }
		copy = heap+(index);
		...
		hashtable_insert(localtables[thr_id], copy,copy);
  ...
  deleteList(sibgroupslist, blank/* free*/);
  //`  printf("deleted list\n");
 //struct hashtable_iterator * itr;
  ...
  for( i = 0; i < 20; i ++)
	...
	//	printf("got key\n");
	if(hashtable_search(intersection,grp)==NULL)
	  {
  ...
  do{
    unsigned int * grp= (list*)hashtable_iterator_key(itr);
    addToList(sibgroupslist, grp);
  }while(hashtable_iterator_advance(itr));
  free(itr);
...
#pragma omp parallel for default (none) private(locuslist,locusorder,p,itr,grp,locus,sibgroupsmap,lociorder) shared(itemsets,loci,nthr_checked,nthr,blocks)
  ...
  /*  for(locus = 1; locus<= loci; locus ++)
    {
      intersectsibgroups(itemsets[0],itemsets[locus]);
