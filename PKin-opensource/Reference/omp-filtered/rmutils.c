...
void updateTable(struct hashtable * table, int *zlist, int ** heap)
{
	    if( NULL == hashtable_search(table, zlist))
	      {
		int ecount = hashtable_count(table);
		/*if (ecount*blocks >= blocksperthread)
		  { 
		    printf("ERROR: INCREASE BLOCKS PER THREAD\n");
		    exit(-1);
		    }*/
		unsigned int * copy = heap+(ecount*blocks); //heap+(index);
		int i;
		for(i = 0; i < blocks; i ++)
		  {
		    copy[i]=zlist[i];
...
{
  unsigned int * res = calloc(blocks, sizeof(unsigned int));
  int i ;
  for ( i = 0; i < blocks; i ++)
    {
      res[i] = data[i];
...
struct hashtable* intersectsibgroups(list * currentList,list * sibgroupslist)
{
  ...
  unsigned int *copy, *ylist, empty,*xptr,*yptr,*zptr;
  unsigned int zlist[60];
  node * sgcurrent;

  //  int i;
  //  int thr_id=0;
  int i,counter2,startindex,sgcount,ecount,index, counter3;
  sgcount = sibgroupslist->count;
  struct hashtable ** localtables;
  unsigned int ** localheaps;
  localtables = calloc(maxthreads,sizeof(struct hashtable *));
  for(counter = 0; counter < maxthreads; counter++)
    {
      localtables[counter]=NULL;
    }

  localheaps = calloc(maxthreads,sizeof(unsigned int *));
  for(counter = 0; counter < maxthreads; counter++)
    {
  ...
  int blocksperthread = sibgroupslist->count*currentList->count*2*blocks;

#pragma omp parallel for default(shared) private(thr_id,i,copy,zlist,index,ecount,xptr,yptr,zptr,counter, xlist,ylist,empty,sgcurrent,startindex,counter2,counter3,nthr) \
  shared(localheaps,localtables,blocksperthread, blocks,sibgroupslist, currentList,nthr_checked,ptrs,ptrs2,clcount,sgcount) schedule(dynamic)
  for(counter = 0; counter < clcount; counter ++) 
    { 
 	...
 	{ 
      ...
      if(localtables[thr_id] == NULL)
	{
	  localtables[thr_id]= (struct hashtable *) create_hashtable((sibgroupslist->count*currentList->count)/(nthr), getArrayHash, equalsArray); 
	}
      if(localheaps[thr_id] == NULL)
	{
	  localheaps[thr_id]=calloc(blocksperthread,sizeof(unsigned int)); 
	}
      ...
      for(counter2 = startindex, counter3 = 0; counter3 < sgcount ; counter2 = (counter2 +1)%sgcount, counter3++)
	{
...

	  {
	    if(!empty)
	      {
		updateTable(localtables[thr_id], zlist, localheaps[thr_id]);     
	      }
	  }
  ...
  list ** lists = calloc(maxthreads, sizeof(list*));
int j;
#pragma omp parallel for default(none) private(i, grp, cpgrp, itr) shared(localtables,lists,localheaps,maxthreads)
...
{
  unsigned int * p,ptr;
  p = calloc(blocks,sizeof(unsigned int));
...
list* lociConstruct(const char * filename, int loci)
{
  ...
  list ** itemsets2 = calloc(loci+1, sizeof(list*));
  nloci=1;
  sibgroup * grp;
  ...
  int locusorder;
  unsigned int * p;
  list * locuslist,*locuslist2;
  blocks = ceil((double)data.count/(8*(double) sizeof(unsigned int)));
  char locusfilename[128];
#pragma omp parallel for default (none) private(locusfilename,locuslist,locusorder,p,itr,grp,locus,sibgroupsmap,lociorder) shared(itemsets,loci,nthr_checked,nthr,blocks)
  ...
  for(counter = 0; counter < data.count; counter ++)
    {
      ...
      list *single=malloc(sizeof(list));
      single->head = NULL;
      single->count = NULL;
      single->current = NULL;
