...
void updateTable(struct hashtable * table, unsigned int *zlist, unsigned char* heap)
{
		...
		unsigned int * copy = heap+gap; //heap+(index);
		int i;
		for(i = 0; i < blocks; i ++)
		  {
		    copy[i]=zlist[i];
...
struct hashtable* intersectsibgroups(list * currentList,list * sibgroupslist)
{
  ...
  if(maxthreads > 3 && blocksperthread >500)
    {
      blocksperthread = ceil(blocksperthread / ((double)maxthreads));
    }
  else{
  ...
  for(counter = 0; counter < maxthreads; counter++)
    {
      ...
      localheaps[counter]=calloc(blocksperthread,sizeof(unsigned int)); 
...
int j;
#pragma omp parallel for default(none) private(i, grp, cpgrp, itr) shared(localtables,lists,localheaps,maxthreads) schedule(static)
