...
extern void writeResultsFileList(const char * filename, list * setslist);
extern struct hashtable* intersectsibgroups(list * currentList,list * sibgroupslist);
extern int equalsArray(void* x_param, void* y_param, int dummy_unused);
extern void init(int nloci);
...
void testOverlaps()
{
  ...
  unsigned int* l=(unsigned int*)calloc(2, sizeof(unsigned int));
  l[0]=y;
  l[1]=z;
  /*  list * l = newList();
  addToList(l, &y);
  ...
  */
  ...
  unsigned int* l2=(unsigned int*)calloc(2, sizeof(unsigned int));
  l2[0]=z;
  l2[1]=y;
  ...
  unsigned int* l3=(unsigned int*)calloc(2, sizeof(unsigned int));
  l3[0]=z;
  l3[1]=z;

  unsigned int* l4=(unsigned int*)calloc(2, sizeof(unsigned int));
  l4[0]=x;
  l4[1]=y;
...
{
  nloci = 2;
  // APR - passing nloci param... not sure what's supposed to go there as init() doesn't exist
  init(nloci);
  dolog = 1;
