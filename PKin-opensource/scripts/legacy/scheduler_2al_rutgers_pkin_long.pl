...
# Output paths
$intermediate_dir=$depot_dir."intermediate/long/";
...
{ 
	...
	while (1)
	{
		...
		my $get_requests_new = $dbh->prepare("SELECT * FROM requests_new WHERE status=5 ORDER BY request_id ASC ");
		...
		while($get_requests_new->fetch())
		{
			...
			if ( $count > 6000 )
			{
				# Set status to 7 to let next scheduler pick up the request
				status( "Pushing $id to next queue" );
				my $update = $dbh->prepare("UPDATE requests_new SET status=7 WHERE request_id= $id");
				$update->execute;
			...
			# Set to -1 for better load balancing (this is key!)
			...
			my $update = $dbh->prepare("UPDATE requests_new SET status=6 WHERE request_id= $id");
			$update->execute;
