...
# Output paths
$intermediate_dir=$depot_dir."intermediate/extralong/";
...
{ 
	...
	while (1)
	{
		...
		my $get_requests_new = $dbh->prepare("SELECT * FROM requests_new WHERE status=7 ORDER BY request_id ASC ");
		...
		while($get_requests_new->fetch())
		{
			...
			my $update = $dbh->prepare("UPDATE requests_new SET status=8 WHERE request_id= $id");
			$update->execute;
