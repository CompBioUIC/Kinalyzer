...
# Output paths
$intermediate_dir=$depot_dir."intermediate/medium/";
...
{ 
	...
	while (1)
	{
		...
		my $get_requests_new = $dbh->prepare("SELECT * FROM requests_new WHERE status=3 ORDER BY request_id ASC ");
		...
		while($get_requests_new->fetch())
		{
			...
			if ( $count > 5000 || $loci > 6 )
			{
				# Set status to 5 to let next scheduler pick up the request
				status( "Pushing $id to next queue" );
				my $update = $dbh->prepare("UPDATE requests_new SET status=5 WHERE request_id= $id");
				$update->execute;
