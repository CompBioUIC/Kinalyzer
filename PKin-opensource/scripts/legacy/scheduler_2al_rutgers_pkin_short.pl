...
# Output paths
$intermediate_dir=$depot_dir."intermediate/short/";
...
{ 
	...
	while (1)
	{
		...
		while($get_requests_new->fetch())
		{
			...
			if ( $linesbyloci > 2000 )
			{
				# Set status to 3 to let long scheduler pick up the request
			...
			{
				# Set status to 3 to let long scheduler pick up the request
				status( "Pushing $id to long queue" );
				my $update = $dbh->prepare("UPDATE requests_new SET status=3 WHERE request_id= $id");
				$update->execute;
				# Skip to next request
				next;
			...
			# Set to -1 to allow better load balancing (this is key!)
