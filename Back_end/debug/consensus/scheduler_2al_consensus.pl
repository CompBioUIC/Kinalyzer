...
sub main() {
	...
	while(1) {
		...
		while($get_requests_new->fetch())  {
			...
			system("/home/kinalyzer/www/consensus/sets test $loci /home/kinalyzer/www/uploads_new/".$id."_new output 2>>errstr.txt 1>>errstr2.txt");
			if(not -e "output.gms")	{
			...
			#close(OUT);
				...
				if(not -e "output.lst")	{
					...
					system("perl /home/kinalyzer/www/newscripts/extract-solution-param.pl output.lst 2>>errstr.txt 1>>errstr2.txt");
					if(not -e "output.sol")	{
						#print to errorlog file
						...
						system("perl /home/kinalyzer/www/newscripts/write-solution.pl -i output -s output.sol -o output.soln 2>>errstr.txt 1>>errstr2.txt");
						if(not -e "output.soln")	{
							...
							system("cp output.soln /home/kinalyzer/www/uploads_new/".$id."_soln.txt");
				...
				system("cp /home/kinalyzer/www/uploads_new/".$id."_new .");
				system("/home/kinalyzer/www/consensus/sets autogreedyconsensus $loci ".$id."_new ");
				system("mv consensus_".$id."_new /home/kinalyzer/www/uploads_new/".$id."_soln.txt");
				#system("rm ".$id."_new *.gms *.lst *_set* *.sol*");
				
			}
							
			open(INP, "< /home/kinalyzer/www/uploads_new/".$id."_new");
			$inplinecnt = 0;
			while(<INP>)	{
				@words_temp = split(/,/, $_);
			...
			if(not -e "/home/kinalyzer/www/uploads_new/".$id."_soln.txt")	{
				open(MAIL, "> /home/kinalyzer/www/uploads_new/".$id."_mailop.txt");
				print MAIL "There were some errors and the solution could not be computed.";
				close(MAIL);
			}
			else 	{
				open(SOL, "< /home/kinalyzer/www/uploads_new/".$id."_soln.txt");
				open(SOL2, "> /home/kinalyzer/www/uploads_new/".$id."_mailop.txt");
			...
			$send_to = "To: ".$email."\n";
			$content = "Hi $name! Thank you for using Kinalyzer to run $algo algorithm.\n Here is the Output to the input(testudo.cs.uic.edu/uploads_new/".$id."_new) you had uploaded, for $loci loci. The numbers in each sibs set are individual IDs from the input file. \n The entire original input, submitted input, and the output files will be available for 7 days at: http://testudo.cs.uic.edu/uploads_new/".$id."_mailop.txt. Use the request id and your email address to access the information. \n\n
...
in Wild Populations', Bioinformatics, 23(13) --\n\n";
			#$content = "";
			$outputfile = "/home/kinalyzer/www/uploads_new/".$id."_mailop.txt";
			#$outputfile = "/var/www/uploads_new/".$id."_soln.txt";
